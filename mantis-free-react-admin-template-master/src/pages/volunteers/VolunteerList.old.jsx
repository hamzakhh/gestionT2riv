    ...
    <body>
      ...
      <div class="filters no-print">
        <div class="filter-group">
          <span class="filter-label">Date de génération</span>
          <div class="filter-value">${new Date().toLocaleDateString('fr-FR', {
            year: 'numeric',
            month: 'long',
            day: 'numeric',
            hour: '2-digit',
        ...
        <div class="filter-group">
          <span class="filter-label">Total des bénévoles</span>
          <div class="filter-value">${volunteers.length} bénévole${volunteers.length > 1 ? 's' : ''}</div>
        </div>
        <div class="filter-group">
          <span class="filter-label">Bénévoles actifs</span>
          <div class="filter-value">${volunteers.filter(v => v.status === 'Actif').length} actif${volunteers.filter(v => v.status === 'Actif').length > 1 ? 's' : ''}</div>
        </div>
      </div>
      ...
      <div style="overflow-x: auto;">
        <table>
          <thead>
            <tr>
              <th style="width: 25%;">Bénévole</th>
              <th style="width: 25%;">Coordonnées</th>
              <th style="width: 15%;">Âge / Genre</th>
              <th style="width: 20%;">Date d'adhésion</th>
              <th style="width: 15%; text-align: center;">Statut</th>
            </tr>
          </thead>
          <tbody>
            ${volunteers?.map(vol => {
              const initials = vol.firstName ? 
                (vol.firstName[0] + (vol.lastName ? vol.lastName[0] : '')).toUpperCase() : '??';
              ...
              <tr>
                <td>
                  <div class="user-info">
                    ...
                    <div>
                      <div class="user-name">${vol.firstName} ${vol.lastName}</div>
                      <div style="font-size: 11px; color: #666;">ID: ${vol._id?.substring(0, 8) || 'N/A'}</div>
                    </div>
                  </div>
                </td>
                <td>
                  <div style="margin-bottom: 4px;">
                    ...
                    ${vol.email || '<span style="color: #999; font-style: italic;">Non renseigné</span>'}
                  </div>
                  <div>
                    <span style="color: #666; font-size: 11px;">📞</span> 
                    ${vol.phone || '<span style="color: #999; font-style: italic;">Non renseigné</span>'}
                  </div>
                </td>
                <td>
                  <div style="margin-bottom: 4px;">
                    <span style="color: #666; font-size: 11px;">🎂</span> 
                    ${vol.age ? vol.age + ' ans' : '<span style="color: #999; font-style: italic;">-</span>'}
                  </div>
                  <div>
                    <span style="color: #666; font-size: 11px;">👥</span> 
                    ${vol.gender || '<span style="color: #999; font-style: italic;">Non spécifié</span>'}
                  </div>
                </td>
                <td>
                  ${vol.joinDate ? formatDate(vol.joinDate) : '<span style="color: #999; font-style: italic;">Non spécifiée</span>'}
                </td>
      ...
      <div class="footer">
        ...
        <div class="total">
          Total: <strong>${volunteers.length}</strong> bénévole${volunteers.length > 1 ? 's' : ''} 
          (${volunteers?.filter(v => v.status === 'Actif').length || 0} actif${volunteers?.filter(v => v.status === 'Actif').length !== 1 ? 's' : ''})
        </div>
      </div>
...
const VolunteerList = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('Données du formulaire à envoyer:', Object.fromEntries(formDataToSend));
      ...
      if (editingVolunteer) {
        ...
        const newVolunteer = await volunteerService.createVolunteer(formDataToSend);
        console.log('Nouveau bénévole créé:', newVolunteer);
  ...
  const handlePrintList = () => {
    // Code pour imprimer la liste des bénévoles
  };
    ...
    <MainCard title="Gestion des Bénévoles">
        ...
        <CardContent>
          <TableContainer component={Paper}>
            <Table>
              ...
              <TableBody>
                {console.log('Rendu du tableau - État:', { loading, nbVolunteers: volunteers.length, volunteers })}
                {loading ? (
                  <TableRow>
                  ...
                  volunteers.map((volunteer) => {
                    console.log('Rendu du bénévole:', volunteer);
                    return (
                    <TableRow key={volunteer._id}>
                      ...
                      <TableCell>{volunteer.joinDate ? new Date(volunteer.joinDate).toLocaleDateString('fr-FR') : ''}</TableCell>
                      <TableCell>
      ...
      <Dialog open={openDialog} onClose={handleCloseDialog} maxWidth="sm" fullWidth>
        <form onSubmit={handleSubmit}>
          <DialogTitle>
            {editingVolunteer !== null ? 'Modifier Bénévole' : 'Ajouter un Nouveau Bénévole'}
          </DialogTitle>
          <DialogContent>
